--------------------------------------------------
--      PONG SMB - A LUA SCRIPT FOR MESEN2      --
-- created by KittiBytes - @daniela.lol on BSKY --
--------------------------------------------------
p1_x = 16
p1_y = 90
p1_width = 16
...
----------------------------------------
-- i wrote this script (talkin about everything not this section) like 5 years ago when i was very dumb,
-- the bottom part is basically stolen but with some if checks added to it, its from the "reverse mode" script
-- by upsilandre, its one of the built in scripts of mesen, credit shouldve been here in the first place
	...
	if gamestarted == 1 then
		if settings_mirror_chaos == 1 then
			if mirror_timer < 1 then
				...
				mirror_timer = 240
				emu.write(0x00FF, 0x04, emu.memType.nesInternalRam)
...
function main()
	gamestarted = emu.read(0x0770, emu.memType.nesInternalRam)
	is2player = emu.read(0x77A, emu.memType.nesInternalRam)

	input = emu.getInput(0)
	input2 = emu.getInput(1)

	mario_powerstate = emu.read(0x0754, emu.memType.nesInternalRam)
	mario_x = emu.read(0x03AD, emu.memType.nesInternalRam)
	if mario_powerstate == 0 then mario_y = emu.read(0x03B8, emu.memType.nesInternalRam) end
	if mario_powerstate == 1 then mario_y = emu.read(0x03B8, emu.memType.nesInternalRam)+16 end
	mario_width = 16
	...
	if gamestarted == 0 then
		...
		if settings_menu_open == 0 then
			...
			emu.drawString(2, 2, "Settings - Open with B", 0xffffff, 0xFF000000)
		end
		...
		if settings_menu_open == 1 then
			...
			emu.drawString(2, 2, "Settings - Close with B", 0xffffff, 0xFF000000)
			emu.drawString(2, 12, "Pong Only: " .. settings_pong_only, 0xffffff, 0xFF000000)
			emu.drawString(2, 22, "Hard Mode: " .. settings_hard_mode, 0xffffff, 0xFF000000)
			emu.drawString(2, 32, "Mirror Chaos: " .. settings_mirror_chaos, 0xffffff, 0xFF000000)
			if settings_selected == 11 then emu.drawString(115, 2, "Disables anything SMB to\nallow a normal and fun game\nof Pong! (Includes 2 Player)", 0xffffff, 0xFF000000) end
			if settings_selected == 21 then emu.drawString(115, 2, "Tired of having fun?\nTry Hard Mode instead!\nDon't let the Ball get Mario,\nor something bad happends!", 0xffffff, 0xFF000000) end
			if settings_selected == 31 then emu.drawString(115, 2, "Every few seconds, not just\nthe game, but also your\ncontrols will mirror!", 0xffffff, 0xFF000000) end
		end
	end

	if gamestarted > 0 then
		if settings_pong_only == 1 then
			emu.write(0x000E, 0x0B, emu.memType.nesInternalRam)
		end
		...
		if (ball_y < 0 or ball_y > 231) then
			emu.write(0x00FF, 0x02, emu.memType.nesInternalRam)
			ball_dy = -ball_dy
		end

		if mirror_flip == 0 then
			if (ball_x < (p1_x + p1_width) + 1 and ball_x + ball_width > (p1_x + p1_width) and ball_y < p1_y + p1_height and ball_y + ball_height > p1_y) then
				emu.write(0x00FF, 0x02, emu.memType.nesInternalRam)
				ball_x = p1_x+16
				ball_dx = -ball_dx
			end
		...
		if mirror_flip == 1 then
			if (ball_x < p1_x + 1 and ball_x + ball_width > p1_x and ball_y < p1_y + p1_height and ball_y + ball_height > p1_y) then
				emu.write(0x00FF, 0x02, emu.memType.nesInternalRam)
				ball_x = p1_x-10
				ball_dx = -ball_dx
			end
		...
		if settings_hard_mode == 1 then
			if (mario_x < ball_x + ball_width and mario_x + mario_width > ball_x and mario_y < ball_y + ball_height and mario_y + mario_height > ball_y) then
				emu.write(0x000E, 0x06, emu.memType.nesInternalRam)
			end
		end
		...
		if is2player == 1 then
			if mirror_flip == 0 then
				if (ball_x < p2_x + 1 and ball_x + ball_width > p2_x and ball_y < p2_y + p2_height and ball_y + ball_height > p2_y) then
					emu.write(0x00FF, 0x02, emu.memType.nesInternalRam)
					ball_x = p2_x-10
					ball_dx = -ball_dx
				end
			...
			if mirror_flip == 1 then
				if (ball_x < (p2_x + p2_width) + 1 and ball_x + ball_width > (p2_x + p2_width) and ball_y < p2_y + p2_height and ball_y + ball_height > p2_y) then
					emu.write(0x00FF, 0x02, emu.memType.nesInternalRam)
					ball_x = p2_x+16
					ball_dx = -ball_dx
				end
		...
		if is2player == 0 then
			if mirror_flip == 0 then emu.drawString(10, 225, "P1: " .. p1_score, p1col, 0xFF000000) end
			if mirror_flip == 0 then emu.drawString(10*21, 225, "P2: " .. p2_score, p2col, 0xFF000000) end
			if mirror_flip == 1 then emu.drawString(10, 225, "P2: " .. p2_score, p2col, 0xFF000000) end
			if mirror_flip == 1 then emu.drawString(10*21, 225, "P1: " .. p1_score, p1col, 0xFF000000) end
		end
		if is2player == 1 then
			if mirror_flip == 0 then emu.drawString(10, 225, "P1: " .. p1_vs_score, p1col, 0xFF000000) end
			if mirror_flip == 0 then emu.drawString(10*21, 225, "P2: " .. p2_vs_score, p2col, 0xFF000000) end
			if mirror_flip == 1 then emu.drawString(10, 225, "P2: " .. p2_vs_score, p2col, 0xFF000000) end
			if mirror_flip == 1 then emu.drawString(10*21, 225, "P1: " .. p1_vs_score, p1col, 0xFF000000) end
		end
